...
class MyHouseItemRecyclerViewAdapter(
    ...
    override fun onBindViewHolder(holder: ViewHolder, position: Int) {
        ...
        if (sharedPreferences.contains("selected_items")) {
            ...
            if(currentSet?.contains(item.attractionId.toString()) == true) {
        ...
        holder.checkboxSelect.setOnCheckedChangeListener { _, isChecked ->
            run {
                ...
                if(isChecked) {
                    updateSet?.add(item.attractionId.toString())
                    editor.putStringSet("selected_items", updateSet)
                    editor.apply()
                } else {
                    if(updateSet?.contains(item.attractionId.toString()) == true) {
                        updateSet.remove(item.attractionId.toString())
                        editor.putStringSet("selected_items", updateSet)
                        editor.apply()
                    }
                }
